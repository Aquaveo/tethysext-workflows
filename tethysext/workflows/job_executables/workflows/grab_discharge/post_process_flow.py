...
"""
********************************************************************************
* Name: grab_discharge/post_process_flow.py
* Author: nswain
...
def main(resource_db_session, model_db_session, resource, workflow, step, gs_private_url, gs_public_url, resource_class,
    ...
    dcl_geojson = params_json['Choose Evaluation Points']['parameters']['geometry']
    print('Define Evaluation Points GeoJSON:')
    pprint(dcl_geojson)
    ...
    for series, feature, y_max in zip(combined_series, dcl_geojson['features'], max_y_values):
        feature['properties']['plot'] = {
            'title': f'Hydrograph for {feature["properties"]["point_name"]}',
            'data': series,
            'layout': {
                'autosize': True,
                'height': 415,
    ...
    spatial_result.add_geojson_layer(
        geojson=dcl_geojson,
        layer_id='evaluation_locations',
        layer_name='evaluation_locations',
        layer_title='Evaluation Locations',
        layer_variable='evaluation_locations',
        popup_title='Evaluation Location',
        selectable=True,
        plottable=True,
