...
"""
********************************************************************************
* Name: detention_basin/run_post_detention_basin_scenario.py
* Author: nswain
...
def main(resource_db_session, model_db_session, resource, workflow, step, gs_private_url, gs_public_url, resource_class,
    ...
    dh_datasets = params_json['Define Post Basin Hydrographs']['parameters']['datasets']
    ...
    # 5. Read in model as new scenario
    print('Uploading new scenario to db...')
    model_db_engine = create_engine(cmd_args.model_db_url)
    make_model_db_session = sessionmaker(bind=model_db_engine)
    model_db_session = make_model_db_session()
    new_scenario = ProjectFile()
    new_scenario.readProject(
        directory=project_dir,
        projectFileName=project_file_name,
        session=model_db_session,
        spatial=True,
        spatialReferenceID=model_srid
    )
    print('New Scenario ID: {}'.format(new_scenario.id))
    workflow.set_attribute('new_project_id', new_scenario.id)
    model_db_session.close()
    ...
    # Parse ohl into json
    series_name = 'User Post Routed Basin'
    ...
    series_name = 'User Post Routed Basin - Overland Flow'
    ...
    # Save to file
    print('Saving File... ')
    with open('post_detention_basin_ohl_series.json', 'w') as f:
